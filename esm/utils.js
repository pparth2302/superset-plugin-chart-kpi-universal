...
import { DEFAULT_DECIMAL_PRECISION, DEFAULT_TREND_LABEL_PREFIX } from './constants';
...
export function getPrimaryMetric(metrics, metric) {
...
export function getMetricLabelSafe(metric) {
  return metric ? getMetricLabel(metric) : 'Metric';
}
export function uniqueMetrics(metrics) {
  var seen = new Set();
...
export function parseBoundedNumber(value, fallback, options) {
...
export function isDurationLike(value) {
...
export function parseDurationInSeconds(value, durationUnit) {
  if (!hasMeaningfulValue(value)) {
    return null;
  }
  if (typeof value === 'number' && Number.isFinite(value)) {
...
export function formatDurationValue(value, durationUnit, showPositivePlusSign) {
  if (showPositivePlusSign === void 0) {
    showPositivePlusSign = false;
  }
...
export function findRecordValue(row, label, options) {
...
export function getFirstMetricRawValue(rows, label, excludedKeys) {
...
export function resolveValueType(valueTypeMode, options) {
...
export function toNumericMetricValue(value, valueType, durationUnit) {
...
export function formatMetricValue(value, options) {
  ...
  var precision = (_options$decimalPreci = options.decimalPrecision) != null ? _options$decimalPreci : DEFAULT_DECIMAL_PRECISION;
  ...
  var baseValue = useSupersetFormatter ? getNumberFormatter(numberFormat)(numeric) : formatNumberWithPrecision(numeric, precision, options.showPositivePlusSign);
  return "" + prefix + baseValue + suffix;
}
export function resolveTimeField(rows, configuredLabel) {
...
export function buildSparklineData(rows, options) {
...
export function aggregateSparklineValue(data, aggregationMode) {
...
export function getLatestNumericSparklineDatum(data) {
  return [...data].reverse().find(datum => datum.value !== null && Number.isFinite(datum.value));
}
export function getFirstAndLastNumericValues(data) {
  var numericValues = data.map(datum => datum.value).filter(value => value !== null && Number.isFinite(value));
...
export function getPreviousAndLatestNumericValues(data) {
  var numericValues = data.map(datum => datum.value).filter(value => value !== null && Number.isFinite(value));
  if (numericValues.length < 2) {
    return null;
  }
...
export function determineTrendState(comparableValue, trendMeaning, neutralThreshold) {
...
export function determineTrendDirection(numericValue, neutralThreshold) {
...
export function calculatePercentChange(baseline, latest) {
...
export function computeTrendResult(options) {
  ...
  var formattedValue = numericValue === null ? '--' : isPercent ? formatMetricValue(formatValue, {
    valueType: 'percent',
    decimalPrecision: (_options$decimalPreci2 = options.decimalPrecision) != null ? _options$decimalPreci2 : DEFAULT_DECIMAL_PRECISION,
    ...
    decimalPrecision: (_options$decimalPreci3 = options.decimalPrecision) != null ? _options$decimalPreci3 : DEFAULT_DECIMAL_PRECISION,
    durationUnit: options.durationUnit,
    showPositivePlusSign: options.showPositivePlusSign
  });
  var prefix = ((_options$labelPrefix = options.labelPrefix) == null ? void 0 : _options$labelPrefix.trim()) || DEFAULT_TREND_LABEL_PREFIX;
  return {
